...
export class Document {
  ...
  public setCharacterCodeTable(code_table: number): Document {
    ...
    this._addB([Constants.ESC, Constants.TAB, [code_table]]);
    return this;
  }

  public setPrintWidth(newWidth: number): Document {
    this.width = newWidth;
    this._addB([Constants.GS, [87], Utils.int16ToArray(newWidth)]);
    return this;
  }
  ...
  public drawTable(data: string[]): Document {
    const cell_width = this.width / data.length;
    let text = '';
    for (let i = 0; i < data.length; i++) {
      text += data[i].toString();
      const spaces = cell_width - data[i].toString().length;
  ...
  public align(alignment: TextAlignment): Document {
    const buf = [].concat(Constants.TXT_ALIGNMENT);
    switch (alignment) {
  ...
  public font(family: FontFamily): Document {
    const buf = [].concat(Constants.TXT_FONT_FAMILY);
    switch (family) {
      case FontFamily.A:
        buf.push(0);
        break;
  ...
  public style(styles: Array<FontStyle>): Document {
    const buf: number[][] = [];
    if (styles.indexOf(FontStyle.Bold) > -1) buf.push(TextFormat.BOLD_ON);
    else buf.push(TextFormat.BOLD_OFF);

    if (styles.indexOf(FontStyle.Italic) > -1) buf.push(TextFormat.ITALIC_ON);
  ...
  public linearBarcode(
    code: string,
    type: Barcode1DType,
    options: Barcode1DOptions = new Barcode1DOptions(),
  ): Document {
    const buf: number[][] = [];
    if (options.width) buf.push(BarcodeFormat.WIDTH(options.width));
    else buf.push(BarcodeFormat.WIDTH_DEFAULT);
    if (options.height) buf.push(BarcodeFormat.HEIGHT(options.height));
    else buf.push(BarcodeFormat.HEIGHT_DEFAULT);
    switch (options.font) {
      case BarcodeFont.A:
        {
    ...
    if (
      options.include_parity &&
      type in [Barcode1DType.EAN13, Barcode1DType.EAN8]
    )
  ...
  private _wrapperSend2DCodeData(
    ...
    let buf: number[][] = [];
    const header = Utils.int16ToArray(
      (data != undefined ? data.length : 0) +
        (modifier != undefined ? 1 : 0) +
        2,
    );
    buf = [Constants.GS, [40, 107], header, [code_type, func]];
    if (modifier != undefined) buf.push([modifier]);
  ...
  public qrCode(
    code: string,
    options: BarcodeQROptions = new BarcodeQROptions(),
  ): Document {
    const ct = 49;
  ...
  public pdf417(
    code: string,
    options: BarcodePDF417Options = new BarcodePDF417Options(),
  ): Document {
    const ct = 48;
    ...
    const error = Math.ceil(options.error_ratio * 10);
  ...
  public cashDraw(pin = 2): Document {
    switch (pin) {
      case 2:
        {
  ...
  public cut(feed = 3, partial_cut = false): Document {
    const buf: number[][] = [];
    this.feed(feed);
    if (partial_cut) buf.push(Paper.PART_CUT);
    else buf.push(Paper.FULL_CUT);
    this._addB(buf);
    return this;
  }
  ...
  public setColorMode(color: boolean): Document {
    let buf: number[][] = [];
    if (color) buf = [Color.RED];
    else buf = [Color.BLACK];
    this._addB(buf);
    return this;
  }
  ...
  public reverseColors(reverse: boolean): Document {
    let buf: number[] = [];
    if (reverse) buf = Color.REVERSE;
    else buf = Color.UNREVERSE;
    this._addB([buf]);
    return this;
  }
  ...
  public text(content: string, override_encoding?: number): Document {
    if (!override_encoding) override_encoding = this.encoding;
    this._addB([
      cptable.utils.encode(override_encoding, content),
      [Constants.EOL],
    ]);
    return this;
  }
  ...
  public image(
    image: ESCPOSImage,
    density: BitmapDensity = BitmapDensity.D24,
  ): Document {
    let n = 3;
    if ([BitmapDensity.D8, BitmapDensity.S8].indexOf(density) > -1) n = 1;
